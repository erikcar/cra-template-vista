import { DoubleLink, BottomLink, DataGraph, GraphNode, LinkDirection, TopLink } from "./DataGraph";
...
export function searchData(items, value, path, pdec, field, start){
  ...
  return findData(items, path, value, field, start);
}
  ...
  export function linkParser(node) {
    ...
    if (node.parent) {
      ...
      if (!link) {
        ...
        if (direction === LinkDirection.DOWN_WISE) {
        ...
        else if (direction === LinkDirection.UP_WISE) {
          //const schema = node.schema;
          //per ora non gestisco multi key
          link = new TopLink(node.primarykey, node.primarykey + node.etype, direction);
        }
        else if (direction === LinkDirection.BIDIRECTIONAL) {
